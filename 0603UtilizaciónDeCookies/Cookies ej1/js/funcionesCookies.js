...
function manageCookies(name, value, days = "") {
  if (!name || name == "") {
    showError("Debe introducir un nombre para crear la cookie");
    return;
  }
  if (days) {
    let date = new Date();
  ...
  days != -1 && days >= -1 ? showMessage("Cookie creada: " + name + " = " + value): false;
  days == -1 ? showMessage("Cookie eliminada: " + name) : false;
  clearForm();
}
/* 
...
function readCookies(name) {
  if (!name) {
    showError("Debe introducir el nombre de la cookie que desea ver");
    return;
  }
  ...
  let salir = true;
  for (let i = 0; i < cookiesDatos.length && salir; i++) {
    if (cookiesDatos[i].includes(name)) {
      let cookN = cookiesDatos[i].split("=");
      document.getElementById("value").value = cookN[1];
      salir = false;
    }
  }
  if (salir) {
    showError("No se encontró la cookie con el nombre especificado");
  }
}
...
function deAllCookies() {
  ...
  for (let index = 0; index < cookiesDatos.length; index++) {
    ...
    manageCookies(resultado[0], resultado[1], (days = "-1"));
...
function showMessage(message) {
  ...
  document.body.insertBefore(messageDiv, document.getElementById("form"));
}
...
function showError(error) {
  ...
  document.body.insertBefore(errorDiv, document.getElementById("form"));
}
